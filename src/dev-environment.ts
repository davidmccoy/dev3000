...
function createLogFileInDir(baseDir: string): string {
  ...
  for (const symlinkPath of symlinkPaths) {
    try {
      ...
      console.warn(
        chalk.yellow(`⚠️ Could not create symlink ${symlinkPath}: ${error}`)
      );
    }
  }
...
function pruneOldLogs(baseDir: string, cwdName: string): void {
  try {
    ...
    const files = readdirSync(baseDir)
      .filter(
        (file) =>
          file.startsWith(`dev3000-${cwdName}-`) && file.endsWith(".log")
      )
      .map((file) => ({
        name: file,
...
export class DevEnvironment {
  ...
  constructor(options: DevEnvironmentOptions) {
    ...
    try {
      ...
      try {
        ...
        if (
          gitRemote.includes("vercel-labs/dev3000") &&
    ...
    for (const port of ports) {
      try {
        ...
        if (result) {
          ...
          console.log(
            chalk.yellow(
              `💡 To free up port ${port}, run: lsof -ti:${port} | xargs kill -9`
            )
          );
          throw new Error(
  ...
  async start() {
    ...
    console.log(
      chalk.greenBright(
        `Starting ${this.options.commandName} (v${this.version})`
      )
    );
    ...
    console.log(
      chalk.cyan("☝️ Give this to an AI to auto debug and fix your app\n")
    );
    console.log(
      chalk.cyan(`🌐 Your App: http://localhost:${this.options.port}`)
    );
    console.log(
      chalk.cyan(
        `🤖 MCP Server: http://localhost:${this.options.mcpPort}/api/mcp/mcp`
      )
    );
    console.log(
      chalk.cyan(
        `📸 Visual Timeline: http://localhost:${this.options.mcpPort}/logs`
      )
    );
    if (this.options.serversOnly) {
      console.log(
        chalk.cyan(
          "🖥️  Servers-only mode - use Chrome extension for browser monitoring"
        )
      );
    }
    console.log(
    ...
    this.serverProcess.on("exit", (code) => {
      ...
      if (code !== 0 && code !== null) {
        ...
        if (isFatalExit) {
          ...
          console.log(
            chalk.yellow("💡 Check your server command and logs for details")
          );
  ...
  private async installMcpServerDeps(mcpServerPath: string): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            "MCP server dependency installation timed out after 3 minutes"
          )
        );
      }, 3 * 60 * 1000);
      ...
      installProcess.on("exit", (code) => {
        ...
        if (code === 0) {
          ...
          reject(
            new Error(
              `MCP server dependency installation failed with exit code ${code}`
            )
          );
        }
      });
      ...
      installProcess.on("error", (error) => {
        ...
        reject(
          new Error(
            `Failed to start MCP server dependency installation: ${error.message}`
          )
        );
      });
    });
  ...
  private async waitForMcpServer() {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        const response = await fetch(
          `http://localhost:${this.options.mcpPort}`,
          {
            method: "HEAD",
            signal: AbortSignal.timeout(2000),
          }
    ...
    throw new Error(
      `MCP server failed to start after ${maxAttempts} seconds. Check the logs for errors.`
    );
  }
  ...
  private async startCDPMonitoring() {
    ...
    try {
      ...
      this.logger.log(
        "browser",
        `[CDP] Navigated to http://localhost:${this.options.port}`
      );
    } catch (error) {
      // Log error and throw to trigger graceful shutdown
      this.logger.log(
        "browser",
        `[CDP ERROR] Failed to start CDP monitoring: ${error}`
      );
      throw error;
    }
  }
  ...
  private async gracefulShutdown() {
    ...
    console.log(
      chalk.yellow(
        `🛑 Shutting down ${this.options.commandName} due to critical failure...`
      )
    );
    ...
    await Promise.all([
      ...
      killPortProcess(
        ...
        `${this.options.commandName} MCP server`
      ),
    ]);
    ...
    console.log(
      chalk.red(`❌ ${this.options.commandName} exited due to server failure`)
    );
    process.exit(1);
  }
